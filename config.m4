...
dnl If your extension references something external, use with:
 ...
 PHP_ARG_WITH(pcache, for pcache support,
dnl Make sure that the comment is aligned:
 [  --with-pcache             Include pcache support])
...
if test "$PHP_PCACHE" != "no"; then
  ...
  dnl [
  dnl   PHP_ADD_LIBRARY_WITH_PATH($LIBNAME, $PCACHE_DIR/$PHP_LIBDIR, PCACHE_SHARED_LIBADD)
  dnl   AC_DEFINE(HAVE_PCACHELIB,1,[ ])
  dnl ],[
  dnl   AC_MSG_ERROR([wrong pcache lib version or lib not found])
  dnl ],[
  dnl   -L$PCACHE_DIR/$PHP_LIBDIR -lm
  dnl ])
  dnl
